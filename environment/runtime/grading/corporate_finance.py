    ...
    for spec in gold['criteria']:
        ...
        if task_id == 'task_035':
            ...
            if numeric_association:
                canonical_numeric_fast_path = False
                reference_context = {'metric': numeric_label.replace('_', ' '), 'expected_value': numeric_expected, 'equivalence_rule': 'Accept any unambiguous professional label, abbreviation, table layout, or locally disclosed unit that associates the expected value with this metric. Do not require the authored label or a finite alias-list match.', 'association_rule': 'The expected value must belong to this metric, period, and scenario. Reject an unlabeled number, a value belonging to a different metric, or a conflicting primary value even if the expected number appears elsewhere on the supplied slides.', 'sign_rule': 'Accept a correctly described adverse magnitude, accounting negative, or cash outflow convention; reject a direction that changes the business meaning.'}
            ...
            if numeric_association:
                requirement = f"Evaluate only whether the supplied slide content clearly associates the deterministically verified value with the {numeric_label.replace('_', ' ')} metric for the correct period and scenario. Accept normal professional wording, abbreviations, layout, rounding, and sign conventions. Reject an unlabeled pile of numbers, the right number attached to the wrong metric, an ambiguous binding, or a contradictory displayed primary value."
            else:
            ...
            task_context = {'assignment': 'Complete the existing nine-slide June executive performance review for leadership.', 'central_work': 'Tie the accounting and reporting figures and support the resulting guidance recommendation.', 'grading_boundary': 'Checkable magnitudes are deterministic hard gates. This judge grades only their professional association, or the named narrative, source, control, or decision meaning, within the supplied slides.'}
        else:
            submitted_evidence = ''
...
def _task_068_numeric_fact_present(path: Path, label: str, expected: float, slide_numbers: Iterable[int] | None=None) -> tuple[bool, str]:
    ...
    for context in contexts:
        for literal in _NUMERIC_LITERAL_PATTERN.findall(context):
...
def _task_068_branch_primary_value_status(path: Path, label: str, expected: float) -> tuple[str, str]:
    ...
    for shape in shapes:
        ...
        if len(text) > 80 or _NUMERIC_LITERAL_PATTERN.search(text):
            continue
    ...
    for branch_shape in branch_shapes:
        ...
        for header in header_shapes:
            ...
            literals = _NUMERIC_LITERAL_PATTERN.findall(str(value_shape.text))
            if not literals:
                continue
...
def _task_068_branch_artifact_value(path: Path, label: str, expected: float) -> bool:
    ...
    for context in contexts:
        ...
        if not any((_context_has_alias(context, alias) for alias in metric_aliases)):
            continue
        if any((_task_068_numeric_literal_matches(literal, expected, context=context) for literal in _NUMERIC_LITERAL_PATTERN.findall(context))):
            return True
    return False
    ...
    if label == 'minimum_guidance_update_headroom':
        for context in contexts:
            ...
            if 'guidance' not in normalized or 'trigger' not in normalized:
                continue
            literals = _NUMERIC_LITERAL_PATTERN.findall(context)
            values: list[float] = []
